...
Foam::COxidationKineticDiffusionLimitedRateWithAshSteam<BedType>::
...
COxidationKineticDiffusionLimitedRateWithAshSteam
(
    const COxidationKineticDiffusionLimitedRateWithAshSteam<BedType>& srm
)
:
...
template<class BedType>
Foam::scalar Foam::COxidationKineticDiffusionLimitedRateWithAshSteam<BedType>::calculate
(
    const scalar dt,
    const label celli,
    const scalar d,
...
) const
{
    ...
    dmass = (dOmega_O2*omega_c+dOmega_CO2+dOmega_H2O+dOmega_H2*0.5)*WC_;
