...
Foam::DiBlasiDevolatilisation<BedType>::
DiBlasiDevolatilisation
(
...
:
    ...
    secondReaction_(readBool(this->coeffDict().lookup("secondReaction"))),
    ...
    id_tar(owner.thermo().carrierId("tar"))
{
    ...
    else
    {
        ...
        if (secondReaction_)
        {
            Info << "  tar -> gas A:" << devolKinetic4_[0] << " E:" << devolKinetic4_[1] <<endl;
...
template<class BedType>
void Foam::DiBlasiDevolatilisation<BedType>::calculate
(
    const scalar dt,
    const scalar age,
    const scalar mass0,
...
) const
{
    ...
    if (secondReaction_)
    {
        ...
        if ((1.0-dt*(kappa4+kappa5)) < 0.0)
        {
            ...
            {
                const label id_gas = volatileToGasMap_[i];
                dMassDV[id_gas] += volgas_[i].Y()*tarprodmass*kappa4/(kappa4+kappa5);
            }
            
            //char
            dMassChar += tarprodmass*kappa5/(kappa4+kappa5);
        }
        else
        {
            forAll(volgas_, i)
            {
                const label id_gas = volatileToGasMap_[i];
                dMassDV[id_gas] += volgas_[i].Y()*tarprodmass*kappa4*dt;   
            }
            
            //char
            dMassChar += tarprodmass*kappa5*dt;  
        }
    }
    else
    {
